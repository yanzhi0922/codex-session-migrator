...
const dictionaries = {
  en: {
    ...
    doctor: {
      ...
      defaultProviderMismatch: 'config.toml defaults to {configProvider}, but the active VS Code sessions are on {activeProvider} ({activeCount}). Codex may return an empty thread list until the default provider is aligned.',
      missingDefaultProviderConfig: 'config.toml sets model_provider to {configProvider}, but that provider has no [model_providers.{configProvider}] section.'
    },
    cli: {
      help: {
        ...
        commandRepair: '  repair     Rebuild SQLite / session_index entries for active and archived sessions',
        ...
        flagSource: '  --source <kind>        Override the visibility source kind (default: vscode)',
  ...
  'zh-CN': {
    ...
    doctor: {
      ...
      defaultProviderMismatch: 'config.toml 的默认 provider 仍是 {configProvider}，但当前活跃的 VS Code 会话都在 {activeProvider}（{activeCount} 个）下。默认 provider 不对齐时，Codex 线程列表可能会直接变空。',
      missingDefaultProviderConfig: 'config.toml 把 model_provider 设为 {configProvider}，但缺少对应的 [model_providers.{configProvider}] 配置段。'
    },
    cli: {
      help: {
        title: 'Codex Session Migrator',
        ...
        commandRepair: '  repair     为活动与归档会话重建 SQLite / session_index 索引',
        ...
        flagSource: '  --source <kind>        指定可见性 source（默认：vscode）',
        exampleServe: '  codex-migrate serve --open',
        exampleList: '  codex-migrate list --provider openai --limit 20',
        exampleMigrate: '  codex-migrate migrate --provider openai --target crs --dry-run',
        exampleExport: '  codex-migrate export --provider codexmanager --format markdown',
